...
- 日付: 2026-09-02
- ステータス: 実装済み。セクション 11 に、実装で覆した設計判断を記録
...
セクション 3〜5 のブラウザ配信は**実装後に破棄した**。以下は実際に動かして分かったことと、
その結果採った設計である。

### 11.1 ブラウザではなくエディタのペインにした

ブラウザ版は完成し、実物の damlc に対して動作した（コードアクション、レンダ、
ライブ更新、トークン認証すべて検証済み）。しかし利用者の要求は「Zed で完結させたい」であり、
別ウィンドウに出る時点でそれを満たさない。

したがって HTTP サーバ、SSE、vendoring した `webview.js` / `webview.css`、
`theme.css`、ブラウザ起動、アクセストークンは**すべて削除した**。依存も
`tiny_http` と `getrandom` が消え、`serde_json` だけになった。

代わりにブリッジは HTML を Markdown に変換し、`<project>/.daml/ide/script-results.md`
に書く。エディタは編集していないバッファをディスクの変更で自動リロードするので、
1 回開けば以降はゼロ操作で更新される。`.daml/` は `dpm new` の `.gitignore` に既に入っている。

### 11.2 ファイルは 1 つ（単一ペイン）

Zed に「このファイルを開け」と指示する手段が無い以上、ファイルが増えるほど手動オープンが増える。
そこで 1 ファイルに固定し、最後に要求された Script を映す。別の Script のレンダが届いても
ペインを奪わない。

自動オープンは WorkspaceEdit を使えば可能だが、バッファが未保存状態になり以後の
自動リロードが止まるため、かえって壊れる。採らなかった。

### 11.3 コードレンズはブリッジ自身が取りに行く

当初の設計（セクション 3.1）に書いておきながら実装していなかった。Zed は
`code_lens` が既定オフのため `textDocument/codeLens` を一度も投げず、レンズのキャッシュが
空のままコードアクションが注入されない、という形で表面化した。`didOpen` / `didSave` /
`codeAction` のたびにブリッジが自分で要求し、その応答はエディタに転送せず握り潰す。

### 11.4 ペインの確保はレンダ要求より先

`textDocument/didOpen` を送ってから「どの Script を映すか」を記録していたため、
サーバが速いとレンダ結果が「映していない Script のもの」として捨てられた。
統合テストで再現し、順序を入れ替えて修正した。

### 11.5 トレースログを入れた

エディタはブリッジの stderr をエディタ自身しか見られないログに飲み込む。
「何も起こらない」を切り分けるのに毎回推測が要ったため、`--log PATH` で
1 メッセージ 1 行の記録を残せるようにした。書き換えは before/after のペアで出る。

### 11.6 Zed の実装で確認できた制約

- Zed は **サーバが `executeCommandProvider` で宣言していないコマンドを持つコードアクションを捨てる**
  （`crates/project/src/lsp_command.rs`）。`daml.showResource` を initialize 結果に足すのは必須だった
- コードアクションの kind は階層マッチなので `source.daml.showResource` は問題ない
- Zed は拡張を **rustup の既定ツールチェーン**で `wasm32-wasip2` にビルドする。
  ディレクトリ単位の `rustup override` は効かない
